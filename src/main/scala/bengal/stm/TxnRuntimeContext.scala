...
private[stm] trait TxnRuntimeContext[F[_]] {
  ...
  private[stm] case class TxnResultRetry(retrySignal: Deferred[F, Unit]) extends TxnResult

  private[stm] case class TxnResultLogDirty(idClosureRefinement: IdClosure) extends TxnResult
  ...
  private[stm] case class TxnScheduler(
    ...
    private def idClosureAnalysisRecursion(
      ...
      if (
        (currentClosure != finalClosure && cycles <= maxWaitingToProcessInLoop && waitingBuffer.nonEmpty) || cycles == 0
      ) {
        ...
        newWaiting.flatMap { nw =>
          idClosureAnalysisRecursion(currentClosure.mergeWith(nw._1), finalClosure, nw._2, cycles + 1)
        }
      } else {
        ...
        result <- log match {
                    case TxnLogValid(_) =>
                      F.uncancelable { _ =>
                        ...
                        s.map(v => F.pure(TxnResultSuccess(v).asInstanceOf[TxnResult]))
                          .getOrElse(
                            log.idClosure
        ...
        for {
          ...
          _ <- result match {
                 ...
                 case TxnResultRetry(retrySignal) =>
                   poll {
                     for {
                       _ <- F.race(retrySignal.get, TF.sleep(ex.retryWaitMaxDuration))
                       _ <- ex.submitTxn(this)
                     } yield ()
                   }
